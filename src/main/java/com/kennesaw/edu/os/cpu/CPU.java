...
import com.kennesaw.edu.os.util.Converter;
...
//   public Register accumlator; // Called from helper class 'Register'
   ...
   /**
    * Called to return a instruction from memory
    * @param pc
   ...
   /**
    * Called to decode given instruction and returns an opCode used for 
    * execution of CPU.
    * @param instruct
    * @return (int) opCode
    */
   public int decode(String instruct) {
      ...
      switch(instType) {
        // Arithmetic or logical operation
         case 00:
            sReg1 = Converter.binaryStringToInteger(tmpInstr.substring(8,12));
            sReg2 = Converter.binaryStringToInteger(tmpInstr.substring(12,16));
            dReg = Converter.binaryStringToInteger(tmpInstr.substring(16,20));
            break;
        // Conditional Jump
         case 01:
            bReg = Converter.binaryStringToInteger(tmpInstr.substring(8,12));
            dReg = Converter.binaryStringToInteger(tmpInstr.substring(12,16));
            addr = Converter.binaryStringToInteger(tmpInstr.substring(16));
            break;
        // Unconditional Jump
         case 10:
            addr = Converter.binaryStringToInteger(tmpInstr.substring(8));
            break;
        // IO Operation
         case 11:
            reg1 = Converter.binaryStringToInteger(tmpInstr.substring(8,12));
            reg2 = Converter.binaryStringToInteger(tmpInstr.substring(12,16));
            addr = Converter.binaryStringToInteger(tmpInstr.substring(16));
   ...
   /**
    * Called to execute given opCode command. This is what is called to
    * 'run' CPU.
    * @param opCode
    */
   public void execute(int opCode) {
      ...
      switch (opCode) {
        // OpCode {00} - Read Content of I/P buffer into a accumlator
         case 0:
            ...
            Register.readBuffer(inputBuffer, addr);
            break;
         // OpCode {01} - Writes the content of accumulator into O/P buffer
         case 1:
            System.out.println("**Executing Write OpCode");
            outputBuffer = Register.writeBuffer(outputBuffer, addr);
            break;
         // OpCode {02} - Stores content of a reg. into an address
         case 2:
            addr = Register.getReg(dReg);
            // Acquire lock
            // Memory.writeMemory(addr, Register.getReg((int)addr));
            break;
        // OpCode {03} - Loads the content of an address into a reg
         case 3:
            Register.setReg(Register.getReg((int)addr), dReg); // (int)addr is too large
            // Acquire lock
            // Memory.writeMemory(addr, Register.getReg((int)addr)); // Right context of memory. Write to memory or disk
            break;
        // OpCode {04} - Transfers the content of one register into another
         case 4:
            System.out.println("Executing Move OpCode");
            Register.setReg(Register.getReg(reg1), reg2);
            break;
        // OpCode {05} - Adds content of two S-regs into D-reg
         case 5:
            System.out.println("Executing ADD OpCode");
            Register.setReg(Register.getReg(sReg1) + Register.getReg(sReg2), dReg); // Add values of sreg1 + sreg2 -> dReg
            break;
        // OpCode {06} - Subtracts content of two S-regs into D-reg
         case 6:
            System.out.println("Executing SUB OpCode");
            Register.setReg(Register.getReg(sReg1) - Register.getReg(sReg2), dReg); // Sub values of sreg1 - sreg2 -> dReg
            break;
        // OpCode {07} - Multiplies content of two S-regs into D-reg
         case 7:
            System.out.println("Executing MUL OpCode");
            Register.setReg(Register.getReg(sReg1) * Register.getReg(sReg2), dReg);
            break;
        ...
        // OpCode {09} - Logical AND of two S-regs into D-reg
         case 9:
            ...
            Register.setReg(Register.getReg(reg1) & Register.getReg(reg2), dReg);
            break;
        ...
        // OpCode {0F} - Loads a data/address directly to the content of a register
         case 15:
            ...
            Register.setReg((int)addr, dReg); // Cast value of addr(string) to int and replace dReg
            break;
        // OpCode {10} - Sets the D-reg to 1 if first S-reg is less than bReg; 0 otherwise
         case 16:
            if (Register.getReg(sReg1) < Register.getReg(bReg))
               Register.setReg(1, dReg);
            else {
               Register.setReg(0, dReg);
            }
            break;
        // OpCode {11} - Sets the D-reg to 1 if first S-reg is less than a data; 0 otherwise
         case 17:
            System.out.println("Executing SLTI OpCode");
            if (Register.getReg(sReg1) < (int)addr)
               Register.setReg(1, dReg);
            else
               Register.setReg(0, dReg);
            break;
        ...
        // OpCode {15} - Branches to an address when content of B-reg = D-reg
         case 21:
            ...
            if (Register.getReg(dReg) == Register.getReg(bReg))
               pc = (int)addr / 4; // Jump
            break;
    ...
    @Override
   	public void run() {
      // Only execute run if there is a valid PCB
      ...
      // Get our buffers from the PCB to write to
      ...
      while (jc > 0) { 
         try {
            ...
            pc++; // Update Memory location each process
   ...
   public void fillCache() {
      ...
      cache[pc] = Memory.readMemForCPU(pc);
   }
